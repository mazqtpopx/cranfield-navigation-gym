...
class MarkerPublisher:
    def __init__(self):
        ...
        self.goal_point_publisher = rospy.Publisher(
            "goal_point", MarkerArray, queue_size=3
        )
        self.velocity_publisher = rospy.Publisher(
            "action_velocity", MarkerArray, queue_size=1
        )
        self.subgoal_point_1_publisher = rospy.Publisher(
            "subgoal_point_1", MarkerArray, queue_size=3
        )
        self.subgoal_point_2_publisher = rospy.Publisher(
            "subgoal_point_2", MarkerArray, queue_size=3
        )

    def publish_goal_markers(self, frame_id, x, y, z):
        # Publish visual data in Rviz
        markerArray = MarkerArray()
        marker = Marker()
        marker.header.frame_id = frame_id
        marker.type = marker.SPHERE  # Change to sphere
        marker.action = marker.ADD
        marker.scale.x = 0.7  # Radius of the sphere
        marker.scale.y = 0.7  # Radius of the sphere
        marker.scale.z = 0.7  # Radius of the sphere (uniform for a ball)
        marker.color.a = 1.0  # Opacity
        marker.color.r = 1.0  # Red channel
        marker.color.g = 0.0  # Green channel
        marker.color.b = 1.0  # Blue channel
    ...
    def publish_velocity_markers(self, action, frame_id):
        ...
        if action[0] >= 0:
            quaternion = transform.Rotation.from_euler(
                "z", -action[1], degrees=False
            ).as_quat()
        else:
            quaternion = transform.Rotation.from_euler(
                "z", -action[1] + math.pi, degrees=False
            ).as_quat()
    ...
    def publish_marker_rec(self, rec, frame_id):
        ...
        marker.header.frame_id = (
            frame_id  # Change this to the frame in which your rectangle is defined
        )
